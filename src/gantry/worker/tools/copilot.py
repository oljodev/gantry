...
TREE_ARCHITECT_PROMPT = (
    ...
    "MODEL: assign each node a model from '## Available models' by matching the agent's "
    "job to the model's cost/capability tier — a token-efficient team uses cheap models "
    "for cheap work and reserves strong models for hard work. Judge tiers from the model "
    "names/providers:\n"
    "  - Orchestrators and planners (can_spawn=true, top-level coordination): the "
    "strongest coding/agentic model (e.g. a 'coder'/'plus'/flagship model).\n"
    "  - Read-only or data-gathering workers (scanning files, reading, grep/glob, "
    "collecting status): the cheapest fast model (e.g. a 'flash'/'mini'/'lite' model).\n"
    "  - Debugging / algorithmic / complex-implementation workers (fixing failing tests "
    "or compile errors, hard algorithms, structural refactors): a reasoning model if one "
    "is available (e.g. an 'r1'/'reasoning'/'thinking' model), else the strongest model.\n"
    "Set the chosen model string on EVERY node's `model`. If the catalog does not clearly "
    "cover these tiers, or the user asked for one specific model, use ask_user to confirm "
    "(label the question 'model'). If revising, keep each node's existing model unless the "
    "user asks to change it.\n"
    "SWARM: design orchestrators (can_spawn=true) as autonomous leaders that delegate "
    "in tiny, isolated MICRO-TASKS — one function/file/focused change per worker — so "
    "many run at once on the async engine, and feed each worker only the precise "
    "context it needs (tiny input footprint). For any team that writes or changes code, "
    "include a dedicated `qa-reviewer` worker whose role is to run the tests and "
    "validate the combined changes AFTER the parallel workers finish, and say so in the "
    "leader's system_prompt: each worker commits and pushes its own branch; the leader "
    "spawns the parallel workers, waits, calls merge_child_branches to integrate their "
    "pushed branches into one staging branch (auto-resolving conflicts), then runs "
    "qa-reviewer on that staging branch before finishing.\n"
