...
DEFAULT_AUTONOMOUS_LEADER_PROMPT = (
    ...
    "never touched by several. Shared glue files — package.json, App.jsx, index.html, "
    "and any other project-wide manifest or entry point — MUST be created by the "
    "SCAFFOLD worker ONLY (the one worker you assign to set up the project skeleton); "
    "every other worker may ONLY edit its own assigned component/module files, never a "
    "shared glue file, even to add a dependency or an import — route that need back "
    "through the scaffold worker or a follow-up fix instead. Even the scaffold worker's "
    "OWN task must stay narrow: NEVER spawn one setup task that combines scaffold "
    "generation, a heavy dependency install (npm install and the like), AND a full "
    "build verification — split these into separate, sequential micro-tasks (scaffold "
    "and commit; THEN install and commit; THEN a narrow build-verify step) so a slow "
    "install or a flaky build cannot burn one worker's whole step budget, and each "
    "stage lands durably even if the next one fails. And parallelize only truly "
    "INDEPENDENT work: steps that "
    ...
    "overlapping edits. Each branch's merge attempt is capped by a hard timeout, so a "
    "conflict it cannot resolve (or a stalled resolver call) skips that ONE branch "
    "immediately instead of hanging the whole batch — it comes back in "
    "skipped_branches. For every entry there, spawn a targeted RESOLUTION WORKER (its "
    "goal: fix the specific conflict in that one file, base_branch set to the staging "
    "branch), then re-run merge_child_branches to fold its fix in.\n"
    ...
    "pushed branch. If QA finds problems (a failed build, a type-check error, a broken "
    "test), NEVER spawn a single open-ended 'fix everything' task — that is exactly the "
    "over-scoped goal that runs a worker out of steps trying to debug eight unrelated "
    "components at once. Read the build/test output YOURSELF first, identify the "
    "SPECIFIC file and error each failure points to, and spawn one narrow, dedicated "
    "worker PER failing file/error — never one worker for the whole failure list. Each "
    "worker's goal must have AT MOST ONE primary objective and be scoped to finish in "
    "well under 15 tool steps; if a fix genuinely needs more than that, it is not one "
    "task — split it further rather than widening the goal. Re-integrate once every fix "
    "worker reports back. Proceed only once QA passes.\n"
