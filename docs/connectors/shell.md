...
| D2 | **The environment is captured once per run, from the login shell — on a background thread, and read by the first command that needs it** (*amended 2026-09-17*). Individual commands run in a shell that does not re-read the user's startup files. | The user gets their real `PATH` and their toolchain managers. Gantry does not re-execute arbitrary user startup code on every command, which is both slow and the thing that would otherwise make the classifier meaningless (§6). The capture runs the user's profile, which on a machine with a version manager and a prompt framework is a good part of a second and is allowed five; waiting for it before the window appeared put all of that in front of every start, and nothing before the first command needs the answer (`PendingShellEnv`, docs/dev/performance.md). |
