...
- **Tailwind CSS v4 + shadcn/ui (Base UI).** Components are copied into the repo, so they can be reshaped for a dense desktop UI without fighting a library. No runtime CSS-in-JS, which matters on WebKitGTK. Base UI became the shadcn default in July 2026; Radix remains supported if a component is missing.
...
```
src/
  app/            router, providers (Query, theme), layout (Sidebar + Outlet), keyboard shortcuts
...
| T21 | Session 2's `releases.json` fetched from the release assets vs browsers blocking cross-origin fetches of `github.com/…/releases/download` | The GitHub releases API (which sends CORS headers) is the only lookup; stable asset names stay; `releases.json` is dropped. 14 §7. |
