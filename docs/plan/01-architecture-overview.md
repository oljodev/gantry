...
| `gantry-documents` | `extract` (a document's bytes → its text and where its pages are), PDF on `pdf-extract` + `lopdf`, `sample::pdf` for the tests either side of it | — | No IO and nothing Gantry-specific. Both readers of a document use it: `read_file` and attachment ingest, and M11's knowledge files next. See `docs/connectors/filesystem.md` §5. |
| `gantry-workspace` | `Scope` (roots, canonicalization, sensitive-path patterns), `Fs` (atomic writes, encoding and line-ending preservation, edit journal), `Diff` (`similar` → hunks), `Search` (`ignore` + `grep-searcher`), `Runner` (`tokio::process`, timeouts, streaming output, kill), `CommandClassifier` | core, documents | Shared by the filesystem, code-editor and shell connectors so scope enforcement is implemented once. |
| `gantry-terminal` | `Terminals` (one pty per open terminal tab on `portable-pty`), `TerminalSink`, incremental UTF-8 decoding, bounded scrollback | — | The user's own shell (15 A24, 16 §5). Nothing here is a tool: no model can open, read or type into a terminal. Separate from `gantry-workspace`'s `Runner`, which captures one command for a tool call with its input closed. |
| `gantry-agent` | `TurnManager`, `TurnRunner` (the loop), `Transcript` (append-only builder + per-provider projection), `ContextBudget`, `SystemPromptBuilder` (10), `PermissionEngine`, `Judge`, `Interactions`, `EventSink` + `Batcher`, `TitleGenerator`, `runtime_tools` (access requests, connector search and suggestions, artifacts, skills, memory), `skills` (index, matcher, import), `memory` (store, selector) | core, store, providers, connectors, documents | See 04, 05, 12 and 13. |
...
7. **Finish.** Assistant messages and usage are persisted, `turn.completed` is emitted, the channel closes. After the first exchange in a chat, `TitleGenerator` names the chat with the utility model of 04 §6 — the same one the guard and the compactor use, and the one Settings → Guard overrides — and emits `chats:changed`.
...
| T19 | The instruction "just use oljo.dev as domain" vs the session-3 brief text naming `gantry.oljo.dev` and vs the client-metadata document needing a host that never changes | **Settled the other way 2026-09-18**: the site lives at `gantry.oljo.dev`, which is what the session-3 brief said, and the apex redirects to it. The metadata document keeps `id.oljo.dev`, unmoved — Pages serves one project per host, and the OAuth identity must not move with the marketing site, which is precisely what happened here. 14 §1. |
