...
The test hands the harness the CSP and the `sandbox` attribute it has read out of the frontend source, so the engine run is of the strings the app ships rather than a copy of them; `gantry_agent::artifacts::sandbox` states this section once in Rust, and the rules no hostile artifact can reach — the nonce, the source and origin checks, the message list, the console budget, the `https:` gate, the `unsupported` answer, the loop guard — are held to it there. Remote URLs are under the reserved `.invalid` TLD, so no hole can make the run talk to anyone; that is also why a case naming a CSP directive additionally requires the document to have reported a `securitypolicyviolation` for it, a request dying in the resolver being indistinguishable from a policy refusing it. `GANTRY_SANDBOX_ENGINE=1` turns a missing engine into a failure, which is what a Linux CI job sets; without it the engine tests say they were skipped. And one test runs the suite again with every withheld flag granted and a policy that allows everything, and requires the cases to notice — a conformance run that cannot tell a sandbox from no sandbox passes for ever.

Two of this section's claims did not survive the run. Both have `#[ignore]`d tests naming them:

- **A script can navigate its own frame.** Link interception covers a click on an `<a href>`; `location.href = "https://…"` navigates the artifact's own frame, and neither the sandbox attribute (self-navigation needs no flag) nor the policy (which has no directive for it) refuses. The URL that leaves is egress, which is the one thing `connect-src 'none'` is there to stop. Closing it means the runtime watching for a navigation away from its document and the parent remounting from stored content.
- **`html` artifacts get no loop guard.** The pass is real for `react` — `compile.ts` registers the plugin and every loop shape is instrumented — but `htmlDocument()` only prepends the prelude to the artifact's own document; its `<script>` bodies are never compiled, so nothing is injected. A `while (true) {}` in an `html` artifact ran unguarded for the eight seconds the case allowed it.
...
1. **Loop protection at compile time.** The React transform (§6) runs a Babel plugin that injects an elapsed-time guard into every `for`, `while` and `do` body; a loop running longer than 3 seconds throws `Artifact loop guard`, which surfaces as a runtime error. Inline scripts in `html` artifacts were to get the same pass when they parse — **not built**: only `react` is compiled, and an `html` artifact's scripts run as written (the conformance run above).
