...
2. **Teardown.** Hidden artifacts (closed tabs, collapsed panel) are unmounted and remounted on demand from their stored content. As built this needs no timer: the right pane renders the active tab's content and nothing else, so a hidden artifact's frame goes at once rather than after 60 seconds. The toolbar's **Stop** (built 2026-09-16) is the one for the tab you are looking at: it takes the frame out of the tree, which is what stops it — a hidden frame is not a stopped one — and the panel shows a Run in its place, which mounts a new document from the stored content and starts it from the beginning. A version that had not reported its render yet is reported as stopped rather than left hanging, so the model that asked for it gets an answer. What Stop is *not* is a rescue from a frozen window: on WebKit the artifact shares the main thread with the app, so an artifact that spins takes the click with it. That is what mitigation 1 is for, and why it now covers `html` too.
