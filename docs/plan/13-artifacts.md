...
- **`html` artifacts get no loop guard** (still open, with an `#[ignore]`d test naming it). The pass is real for `react` — `compile.ts` registers the plugin and every loop shape is instrumented — but `htmlDocument()` only prepends the prelude to the artifact's own document; its `<script>` bodies are never compiled, so nothing is injected. A `while (true) {}` in an `html` artifact ran unguarded for the eight seconds the case allowed it.
