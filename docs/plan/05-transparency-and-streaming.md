...
| `tool_call.output` | call_id, stream (stdout\|stderr\|log), chunk | **built 2026-09-08** as the transient half: the batcher merges consecutive chunks of the same call and stream, and the run store keeps the 400-line window. **2026-09-13**: the same 400-line window is kept on the turn while a call runs and travels in `turn.snapshot`, so a view that reattaches mid-command sees what has been printed rather than an empty row — transient means "not in the transcript", not "unavailable to a view that arrived late". A finished call keeps none: its result carries the output. The blob and its checkpoints arrive with the result-capping work; until then a command's full log is whatever its result carries |
