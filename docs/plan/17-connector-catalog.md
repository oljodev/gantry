...
| Google Cloud (**27 services**, re-probed 2026-09-21; **not** any Workspace product) | `https://<service>.googleapis.com/mcp` | 200 | Google OAuth, device flow, client id you supply — the `github` shape (B8) |
...
| Google Workspace — Gmail, Drive, Calendar, Docs, Sheets, Slides | Nothing to connect to, re-checked 2026-09-21. `gmail`, `drive`, `calendar`, `admin`, `people`, `tasks`, `keep`, `chat` and `forms` all answer 404 at `/mcp`; `docs`, `sheets` and `slides` answer with Google's HTML web frontend rather than an MCP server; `mcp.google.com` does not resolve and `workspace.googleapis.com` is 404. Google ships MCP for Cloud (§A, B8) and not for Workspace. Community servers exist; see the note under §3 |
...
| **B6** ✅ | Local, Node | Playwright, Shopify Dev, Azure, Salesforce, **Make** | Shipped 2026-09-12. BrowserStack came out for the two-credential reason B5 found. Azure and Salesforce need no credential at all — they borrow the sign-in `az login` and the Salesforce CLI already left on the machine, which is a fifth auth shape in practice: *whatever your CLI can do*. Make is the first entry combining a `user_config` form (zone, team) with a vault-held key. **Still one install from Olav**: the runtime check and the stderr log have unit tests and have never met a real `npx` |
| **B7** ✅ | Local, Python | ElevenLabs, Qdrant, MiniMax | Shipped 2026-09-12, three rather than five: Kagi and Ramp are in the catalogue already as remote servers, which is the better way in for both. The probe now asks **PyPI** about a `uvx` package as it has always asked npm about an `npx` one — before this batch a Python server was checked against nothing at all. **One install from Olav** |
| **B8** | Remote, OAuth client you supply | Google Cloud, 27 services | **Not shipped, but not blocked — it is the `github` shape.** Re-probed 2026-09-21 and this row was wrong twice over. It is not "a handful": **27 services answer** and serve their tool list unauthenticated — `bigquery`, `run`, `compute`, `logging`, `monitoring`, `cloudresourcemanager`, `firestore`, `pubsub`, `sqladmin`, `container`, `apigee`, `iam`, `redis` (25 tools), `cloudbilling` (29), `alloydb` (17), `dataproc` (16), `bigtableadmin` (14), `datastream` (10), `file` (8), `serviceusage` (8), `cloudasset` (4), `networkmanagement` (4), `discoveryengine` (3), `cloudtrace` (2), plus `spanner`, `secretmanager` and `dataplex` on an older revision. `aiplatform`, `cloudfunctions`, `artifactregistry`, `observability`, `cloudkms`, `securitycenter` and some twenty others are 404; `storage` and `cloudbuild` answer 400 from something that is not an MCP server. <br>**The auth shape is not the `github` one this row assumed.** A tool *call* answers `401` with `WWW-Authenticate` pointing at a **per-tool** protected-resource document (`/.well-known/oauth-protected-resource/<tool>`, which names that tool's exact scope; the bare path is 404). It points to `accounts.google.com`, and that metadata offers **no `registration_endpoint`, no `client_id_metadata_document_supported`, and `token_endpoint_auth_methods_supported` of `client_secret_post` and `client_secret_basic` only**. That reads like Slack's position and **it is not** — corrected the same day, before anything was written. Google also advertises `device_authorization_endpoint: https://oauth2.googleapis.com/device/code`, and `auth::prefers_device` already routes exactly this combination — a device endpoint, and no `none` among the token endpoint's methods — to the code flow. That is the rule §7 of 03 wrote for **GitHub**, which is shipped and works. So B8 is the `github` shape: `registration: ["preregistered", "user_supplied"]`, `user_supplied_fields: ["client_id"]`, and instructions telling the user to create an OAuth client of the *TVs and Limited Input devices* type in their own Google Cloud project and paste its id. No mechanism is missing. <br>**What is left to confirm, and it needs one real client id:** whether Google's device *token* poll also wants a client secret. If it does, the gap is small and specific — `user_supplied_fields` already allows `client_secret` in the schema, `flow.rs` already sends one and the vault already holds an `oauth_client_secret` per issuer (built for Supabase, 03 §7), but only `needs_client_id` is carried to the interface, so there is no box to type a secret into. A *user-supplied* secret in the user's own vault is not the thing 03 §7 refuses; shipping Gantry's own secret is |
