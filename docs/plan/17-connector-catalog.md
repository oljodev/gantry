...
| Google Cloud (**27 services**, re-probed 2026-09-21; **not** any Workspace product) | `https://<service>.googleapis.com/mcp` | 200 | Google OAuth, and see B8 — the client cannot be public |
...
| **B8** | Remote, OAuth client you supply | Google Cloud, 27 services | **Not shipped, and blocked on the confidential-client question below, not on probing.** Re-probed 2026-09-21 and this row was wrong twice over. It is not "a handful": **27 services answer** and serve their tool list unauthenticated — `bigquery`, `run`, `compute`, `logging`, `monitoring`, `cloudresourcemanager`, `firestore`, `pubsub`, `sqladmin`, `container`, `apigee`, `iam`, `redis` (25 tools), `cloudbilling` (29), `alloydb` (17), `dataproc` (16), `bigtableadmin` (14), `datastream` (10), `file` (8), `serviceusage` (8), `cloudasset` (4), `networkmanagement` (4), `discoveryengine` (3), `cloudtrace` (2), plus `spanner`, `secretmanager` and `dataplex` on an older revision. `aiplatform`, `cloudfunctions`, `artifactregistry`, `observability`, `cloudkms`, `securitycenter` and some twenty others are 404; `storage` and `cloudbuild` answer 400 from something that is not an MCP server. <br>**The auth shape is not the `github` one this row assumed.** A tool *call* answers `401` with `WWW-Authenticate` pointing at a **per-tool** protected-resource document (`/.well-known/oauth-protected-resource/<tool>`, which names that tool's exact scope; the bare path is 404). It points to `accounts.google.com`, and that metadata offers **no `registration_endpoint`, no `client_id_metadata_document_supported`, and `token_endpoint_auth_methods_supported` of `client_secret_post` and `client_secret_basic` only**. That is the same confidential-client position as Slack, HubSpot and Microsoft Fabric. <br>**What would settle it:** one real Google "Desktop app" client id. Google's metadata is known to under-describe installed-app clients, and a token exchange probed with a fabricated id cannot distinguish "secret required" from "client not found" — both answer `invalid_client`. If a Desktop-app client completes PKCE without a secret, the batch is the `github` shape and is writable; if it does not, B8 belongs in the paragraph below with Slack |
