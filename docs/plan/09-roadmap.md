...
- ~~The attach menu complete: files, folder (with the install-all-three dialog), project, connectors checklist, web search toggle (provider server tools with opaque-part rendering), thinking selector.~~ **Done 2026-09-13**; its `web` connector fallback **done 2026-09-16**, once that connector had a keyless `search` for it to fall back to. Five things worth recording:
  ...
  - **The toggle's second state, and what it turned out to be.** A model without `server_web_search` got a greyed-out switch saying "Not on this model" — true about the model, false about the app, and on a machine with no provider key it said it about every model including the ones that do have it. The switch now asks the `web` connector instead. Not the way this line first imagined it, which was the runner calling that connector's `search` behind the `web_search` flag: that would be a tool call nobody attached, nobody can see in the connector list, and no permission covers. Turning the switch on **attaches the connector to the chat** — install it if this machine has none, connect it, attach it — so the model gets `search` and `fetch_url` as tools like any others, and the feed, the permission engine and the connector checklist all say so. Turning it off detaches it. Where the provider has its own search it still wins and the switch still means that: one round trip inside a call the user is already paying for beats two tool calls. The `+` menu names which one it will use. The welcome screen, which has no chat yet to hold a provider flag, holds the connector with the folders it already holds until the first message makes one.
- ~~`desktop/connectors/web` (`fetch_url`, optional `search` with a BYOK search key).~~ **Done 2026-09-13**, `fetch_url` only: the BYOK half was built and then removed the same day.
  ...
  - **The search key was the wrong answer to the right question, and came out again.** A `search` tool over Brave, Tavily or Exa was built, wired to the vault and shipped behind a settings form — and then removed, because this connector is free and local by rule (`docs/connectors/web.md` §1: "No key field, no free tier to exhaust, no upsell"). The M11 line above asked for BYOK and the rule outranks it. What survives is the shape: `Connector::tools` stays computed, so a keyless `search` can appear later without a manifest change, and the manifest now asserts in a test that it asks for nothing, since a `user_config` key field is exactly the sort of thing that comes back by accident. What replaces it is §6 of that document — a query router over free purpose-built APIs, the user's own SearXNG, a rationed general engine, then independent indexes. The honest ceiling is recorded there too (§6.7): one search and several reads per question works forever and free; an agent that wants five searches while reasoning will feel the ration.
  - **Keyless search, built the same day the key came out.** §6.2's tier 0: Wikipedia, Stack Overflow, crates.io and npm, all without an account, routed by keyword over the query, asked concurrently and interleaved so each index shows near the top. The rest of §6 — the user's own SearXNG, a rationed DuckDuckGo Lite, mwmbl and Wiby below that — is not built, so a query none of the four covers returns a sentence naming what it can search. Never an empty list: §6.8's rule is the important one, because `[]` reads to a model as proof the thing does not exist and it will answer from memory and cite nothing.
  - **General web search, rationed rather than pretended.** §6.4 and the top of §6.5: DuckDuckGo's Lite endpoint for everything the four curated indexes do not hold — documentation, release notes, blog posts, anything recent — with mwmbl behind it. The measurement in §6.1 is what shapes the code: the engine blocks after five or six queries in two minutes, for twenty minutes, so the budget is enforced here rather than discovered by the user. One query per 25 s, claimed under a lock because this connector is `parallel_safe` and five searches in one turn is the normal case; a seen block opens a twenty-minute circuit breaker, since retrying into one extends it; anything refused falls to mwmbl, which is never rationed. A no-results page is deliberately not treated as a block — that distinction is the difference between "try different words" and "stop asking for twenty minutes". §6.1's other finding saved a dependency: the block is triggered by request headers rather than TLS fingerprint, so an ordinary complete browser header set is served normally and no fingerprint-impersonating client is needed. Not built: a per-turn cap, SearXNG (§6.3), Wiby and YaCy.
  - **`find_in_page`, so a long page is read once at the right place.** Paging made a 287 000-character article reachable but still sequential — seven reads at the default budget, six of them wasted if the answer is in the last section. This is the index: a pattern gives every matching line with its character offset and the heading it sits under, no pattern gives the headings themselves, and every offset goes straight back to `fetch_url`. It costs no request, because it reads the document already in the cache. The model that tested the connector asked for exactly this, and `docs/connectors/web.md` §5 had asked for a version of it first — a `query` parameter returning ranked passages. The deterministic half is built; ranking can sit on top of it. One tool rather than the two proposed, because a search with no pattern *is* an outline and every tool costs context on every turn. The correctness condition is that both tools measure offsets in the same rendering of the same document, which is why they share one `format` argument and one cache, and why the test that reads from each offset and checks a heading is there exists.
  - **A registry has to be asked by name as well as searched.** The first model to use `search` in anger reported crates.io returning `reqres` and `pyo3-async-runtimes` for a Tokio question and not `tokio`. Both registries match every query word against name *and* description, and tokio's description never says "runtime", so it is absent from thirty results and there is nothing to re-rank. Each search is now paired with an exact by-name lookup of up to three words from the query; a word that is not a package answers 404 and costs nothing. `dominant` then drops the incidental namesakes — `runtime` exists, at 0.0.0 and a ten-thousandth of tokio's downloads — by a relative rule rather than a threshold, because "popular" is not a number that holds across ecosystems.
  - **The fixtures are real this time, and that is the whole difference.** The BYOK search was built against hand-written Brave, Tavily and Exa responses and never met a live one; its parsers were fiction that compiled, and nothing would have caught it until the user pasted a key. Every fixture here was captured from the live API and saved verbatim. More to the point, a `#[ignore]`d live test runs the real connector against the real indexes — and it found two bugs that no recorded response could have: a registry was being searched for the word that routed the query to it (`serde crate` ranked `serde_core` above `serde`), and sentences were being run through registries at all (`cannot borrow as mutable more than once rust` returned `fp-bench`). A recorded reply tests the parser. It cannot tell you the request was wrong.
  - **Removing it took the native `user_config` path with it.** `NativeConfig`, `ConnectorService::native_config` and the vault round-trip test went in the same commit: `web` was their only consumer, and no other native connector asks the user for anything. `set_user_config` still files a `sensitive` answer in the vault for any connector whose manifest declares one — that branch is correct and now has no caller, so it is untested until something needs it. `git log` has the deleted machinery; §6.3's SearXNG URL is the likely reason it comes back, and that one is a plain URL rather than a secret.
  ...
  - **A long page is a page turn rather than a truncation.** `fetch_url` returns a window — `offset` in, `first_char`, `total_chars`, `more` and `next_offset` out — and the document it was cut from is kept for five minutes, so reading on is neither a second download nor a second parse, and the offsets cannot go stale under the model between two calls. The names are `filesystem.read_file`'s rather than the reference MCP fetch server's `start_index`/`max_length` that `docs/connectors/web.md` proposed: a model holding both tools in one turn should meet one convention, and `max_chars` had shipped already. The same memory is what makes reading one URL twice in a chat cost one fetch. It is bounded in pages *and* in characters, because one 5 MB page and eight small ones are the same problem, and a page larger than the whole cache is not kept at all rather than evicting everything to make room for itself.
  ...
  - **A native connector's recorded tool list follows the build.** For an MCP server the cached list is the last thing the server said and only a connection changes it; a native connector's tools come from the code, so a list from an older build is simply wrong. `rebuild` reconciles the two and writes only when they differ. Without it a `web` installed from the release whose manifest declared no tools reads "No tools yet" for ever — the catalogue ships inside the app (03 §11), so "installed before its code existed" is the ordinary upgrade path and not an edge case.
  - **The `user_config` form now exists for a connector that is already installed.** M9's list (above) promised a "settings form from `user_config`" and the form was built — inside the install dialog only. That leaves two states unreachable: a connector installed *before* its manifest asked for anything, and an answer that has to change later. `web` was both at once, and the effect was the whole of BYOK being unreachable — the `search` tool compiled in, `set_user_config` writing to the vault, `native_config` reading it back, and no box anywhere to type the key into. `ConnectorSettings` in `ConnectorsSection.tsx` renders the same `ConfigForm` on an expanded row when the manifest asks for something. The connector that prompted it no longer asks for anything, and the component outlived it: `make`, `minimax` and `qdrant` all declare `user_config`, and until this existed none of them could be configured after install either. The connector's own error messages said "Settings → Connectors" and the menu says **Customize** (15 A18 moved it); they say Customize now.
...
- ~~Context management: tool-result caps, client-side simple compaction, keep-tail compaction for other providers, the "summarized" notice.~~ **Done** 2026-09-11, pulled forward out of M13 because it is what bites first in ordinary use: a long chat simply stopped working. `gantry-agent/src/context.rs` with the summarizer prompt in `assets/prompts/compaction.md`, the decisions recorded in 02 §6 "As built". The tool-result cap became a setting (Settings → Advanced). ~~Still in M13: … the full tool output as a blob~~ **done 2026-09-16** — and the cap had been losing the middle outright, under a comment saying the drawer still had it. **Anthropic's server-side context editing and compaction stay unbuilt**, as a decision rather than a debt (02 §6): they are beta request shapes that cannot be verified without an Anthropic key, and a shape nobody has sent is a shape nobody knows is wrong. What would unblock them is a key, one live capture, and a fixture cut from it.
- ~~The Anthropic append-only conformance check (the three-step check with `prefix_mismatch_behavior: "error"` in a test, `drop_block` in production where a tool set had to be rebuilt); prompt-cache hit verification in usage; the memory and instruction `SystemNote` paths included.~~ **Done 2026-09-16** (02 §6 "The prefix, and what happens when it moves"). The three-step check is offline and compares bytes — one chat, an instruction change, then a connector attached mid-conversation — which is a stronger everyday assertion than asking a server to refuse us; `"error"` is what the live run sends, where the only party that knows what it bound a block to is the one answering. `drop_block` goes out only once the tool array really was rebuilt, and `input_transformations` comes back as a live `provider.notice`. The cache scenario in the live run used to print `cache_read` and pass whatever it was, which made the one check that catches a drifting prefix incapable of failing; it fails now, and the number reaches the user in the turn footer.
- ~~The automated sandbox conformance test for artifacts (13 §5).~~ **Done** 2026-09-13.
  ...
  cases run with the rest. So is the third, the toolbar's **Stop** (hang risk 2), which was
  never built and now takes a running artifact's frame out of the tree. Nothing in §5 is
  `#[ignore]`d any more.
- ~~A fifth first-party connector, `media`: the chat model makes a picture, a voice-over or a clip as one step of its own work.~~ **Done 2026-09-17**, not from this milestone's list — Olav asked for it. It is the other direction from 02 §4b's routing and reuses all of it: a `ChatRequest` whose only message is the prompt, through `Provider::stream`. One tool, one generation per call, `write_external`, and not attached by default. What it needed from the rest of the app was one new thing, and that is the part worth remembering: `ToolOutcome::media` (03 §4), so a tool can put something in the **answer** rather than only in its result. A picture described in a tool result and left for the model to quote is a picture nobody sees; as an assistant message of its own it renders at the point of the call, is parked in the blob store, is reachable by the sweeper, and is dropped by every provider on the way out.
...
- Crash recovery and cancellation tests across all connectors; ~~the blob sweeper~~ **done 2026-09-16**; database backup before migration. The sweeper was the smaller half of that item: nothing swept, and four paths had left bytes behind that a sweeper reading the `refcount` column would not have found either — project knowledge files, a document's extracted text, every `before` and `after` in the edit journal, and media parts. The count is gone (0015) and the sweep reads the referencing tables instead, so a missed release is impossible rather than invisible (06 §3, §8).
...
- Packaging: macOS signing and notarization, Windows signing (NSIS), ~~Linux AppImage/deb/rpm~~, `tauri-plugin-updater` with a static release feed; ~~the stable-named release assets~~ ~~and `releases.json` step (14 §3)~~; ~~`THIRD_PARTY_LICENSES.md`~~. **`release.yml` and the notices done 2026-09-22.** A `v*` tag checks that the tag, `tauri.conf.json`, the workspace `Cargo.toml` and the frontend agree on the version and that the commit is on main; builds Linux (AppImage, deb, rpm) and Windows (NSIS); copies each bundle to the stable name the site's buttons ask for; and puts them on a **draft** release with `SHA256SUMS`, the notices, and the changelog entry's body as notes. Publishing stays a hand's job, because it is the step that turns every download button on. `releases.json` was never needed — T21 in 01 replaced it with the releases API — and this line had outlived that. The notices are `cargo xtask licenses`: 535 crates through cargo-about, 336 JavaScript package versions through `pnpm licenses` with each package's own licence files, the two fonts and Simple Icons, and a paragraph for the 162 Ubuntu libraries the AppImage carries (WebKitGTK, GTK, GStreamer and the rest, checked by extracting the 2026-09-10 build). The file is installed with the app as a bundle resource and the release refuses to build when it is stale. Two things neither can prove from here: **Windows has never been built in CI** — `full-matrix` was never run — so the first run on a public repository should be Build with that box ticked, before any tag; and the workflow itself cannot run until the repository is public. Signing and the updater are still open, and optional for a first release. ~~The M0b onboarding wired to real settings (add a key, choose a theme, add a folder and install the local connectors).~~ **Done 2026-09-16** — and it was worse than "not wired": the screen was unreachable (nothing navigated to `/onboarding`, no first-run flag existed), the typed key was discarded under a label promising it was stored, the theme was overwritten by the backend a second later, and the folder button had no handler. Installing the local connectors is deliberately *not* part of it: the file tools are offered at the moment a folder is added instead (03 §11), which is where the question makes sense. macOS signing and notarization are dropped with macOS itself — v0.1.0 is Linux and Windows only (decided 2026-09-16).
