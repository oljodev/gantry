...
- ~~The attach menu complete: files, folder (with the install-all-three dialog), project, connectors checklist, web search toggle (provider server tools with opaque-part rendering), thinking selector.~~ **Done 2026-09-13**, except the `web` connector fallback, which is still not built. The connector it was waiting for is the next line and exists now, but nothing routes the toggle to it: `runner.rs` turns `web_search` into a `ServerTool::WebSearch` and that is the whole of it, so a model without `server_web_search` shows "Not on this model" and reaches for nothing. Wiring it means the toggle asking the `web` connector's `search` when the provider has no search of its own, and that tool does not exist yet — the BYOK one built for it was removed (see the next line), and its keyless replacement is `docs/connectors/web.md` §6. So "Not on this model" stays literally true for now, and the toggle's second state is waiting on that work rather than on a key. Four things worth recording:
