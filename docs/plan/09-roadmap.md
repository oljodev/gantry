...
- ~~`desktop/connectors/web` (`fetch_url`, optional `search` with a BYOK search key).~~ **Done 2026-09-13**, with one piece left wired to nothing; see the last point.
  ...
  - **The search key is not yet plumbed to the vault.** The manifest declares `SEARCH_API_KEY` as `sensitive`, so `set_user_config` already puts it in the vault and keeps it out of the config row (03 §11 step 2) — but `native::build` is handed the workspace and the shell environment and no `SecretVault`, so it constructs `Web` with no key and `search` is never offered. Closing it is one more parameter on `native::build`, filled in at the `rebuild` in `connectors.rs` that calls it. `fetch_url`, the half that needs no account, works now.
