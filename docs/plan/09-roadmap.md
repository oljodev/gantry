...
- Packaging: macOS signing and notarization, Windows signing (NSIS), Linux AppImage/deb/rpm, `tauri-plugin-updater` with a static release feed; the stable-named release assets and `releases.json` step (14 §3); `THIRD_PARTY_LICENSES.md`. ~~The M0b onboarding wired to real settings (add a key, choose a theme, add a folder and install the local connectors).~~ **Done 2026-09-16** — and it was worse than "not wired": the screen was unreachable (nothing navigated to `/onboarding`, no first-run flag existed), the typed key was discarded under a label promising it was stored, the theme was overwritten by the backend a second later, and the folder button had no handler. Installing the local connectors is deliberately *not* part of it: the file tools are offered at the moment a folder is added instead (03 §11), which is where the question makes sense. macOS signing and notarization are dropped with macOS itself — v0.1.0 is Linux and Windows only (decided 2026-09-16).
