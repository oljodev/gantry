...
no account, no quota to buy. `docs/connectors/web.md` §1 is the rule; a `search` built on
...
- The 2026-07-28 revision removed protocol sessions and the initialize handshake, replaced server-initiated requests (elicitation, sampling, roots) with the Multi Round-Trip Request pattern (**as built, 2026-09-11**: `McpSession::call` walks the rounds itself rather than using rmcp's `call_tool`, which walks them through the session's `ClientHandler` — a handler is shared by every call a connector makes and is handed no way to tell which one it is answering for, and an elicitation that reaches the wrong card is worse than none. Driving the loop in the session keeps the call, the user and the answer in one scope, and the asking rides on the call's own `ToolEventSink`, which is already scoped to exactly the right call. Only *elicitation* is answered by a person: sampling asks Gantry to run its model on the server's prompt and roots asks which folders it may see, and both are declined with the specification's own word rather than in silence. A URL elicitation is declined too — it sends the user to a web page and Gantry has no way to know when they have finished, so the honest answer beats a wait with no end. Ten rounds, then the call is a loop), deprecated Roots, Sampling, Logging and Dynamic Client Registration, added required HTTP headers (`Mcp-Method`, `Mcp-Name`) and cacheable list results with `ttlMs`. Most servers installed today still speak the handshake-based versions. Supporting both, correctly, and following a spec with a yearly major revision and twelve-month deprecation windows is what an official SDK is for. A custom client would be roughly three thousand lines that start decaying the day they are written.
...
- **Add folder to workspace** in a *chat* with the filesystem connector not installed shows one dialog offering to install it. A chat is never given the code editor or the shell; the surface split (16) is what makes that clean. The Web search toggle used to work the same way; it no longer has to, because `web` is installed by the time the toggle is drawn. It attaches the connector on every model and additionally asks the provider to search where the model can (2026-09-17): the connector is not only search — it reads a page, pages through a long one and finds a pattern in it — and attaching it only where the provider could not search left those three tools unreachable on exactly the models that read pages best.
