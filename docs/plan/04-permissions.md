...
Matching: same chat and instance; tool matches or is wildcard; tier at or under the ceiling when set; `arg_scope` predicate holds (`path_prefix`, `command_prefix`), failing closed when the argument is absent. As built, a card offers up to four standing scopes, narrowest first: **this tool under this folder** when the call names a path, **this tool for commands starting with these words** when it names a command, **this tool for this chat**, and, for a `read` call, **all reads from this connector for this chat** — the "Allow all reads" of §4. The offered path is the file's parent directory and the offered command is the program and its subcommand; a prefix matches only where a name ends, so `/home/olav/dev` never reaches `/home/olav/development` and `cargo test` never reaches `cargo testify`. **A card offers no standing scope at all where one would not be honoured**: a guardrail is never answered by a grant and `always_confirm` asks every time, so those prompts offer Allow once alone — bar the sensitive path this section says an explicit grant may reach, which keeps the argument scope and only that. A grant made in answer to one card applies from the next batch of calls onwards. `App` tools are never granted, because they never ask. Grants live as long as the chat and are listed in the chat's **Permissions** panel (mode, guard, roots, attached connectors, grants with revoke, recent decisions). A "Use as project default" action copies a grant into the project's defaults, which new chats in the project inherit; existing chats are untouched.
...
**A sub agent's card** (18 §6) is an ordinary `Interaction` with the **parent's** `chat_id` and
`turn_id` in it, so it renders in the conversation the user is actually looking at rather than in
a transcript they have to open a modal to find; the payload names the agent and its task. Where
the setting says the guard answers instead, no card is made at all and the sub agent runs in Auto
with the judge. A sub agent never holds more permission than its parent chat.
