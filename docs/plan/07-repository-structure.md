...
│   │   └── release.yml                   # on a `v*` tag: Linux x64 (AppImage, deb, rpm) + Windows x64 (NSIS), stable names,
│   │                                     # SHA256SUMS and the notices on a draft release; published by hand (14 §3)
│   └── ISSUE_TEMPLATE/
...
│   │       ├── routes/                   # TanStack file routes: __root, index, chat.index, chat.$chatId, code.index, code.$sessionId, projects, projects_.$projectId, artifacts, artifact-window, incognito, onboarding, dev.gallery; routeTree.gen.ts is generated. Settings and Customize are dialogs, not routes (15 A18). The trailing `_` in `projects_.$projectId` is not decoration: without it TanStack nests the page under `projects.tsx`, whose component is the list and renders no `<Outlet/>`, so opening a project silently shows the list again
...
│   │   │       ├── interactions.rs
│   │   └── xtask/
│   │       ├── about.toml                # cargo-about's configuration for `licenses`: accepted licences mirror deny.toml
│   │       └── src/{main.rs, connectors.rs, licenses.rs, probe.rs, skills.rs, windows.rs, icons.rs}   # finds the root by walking up to Cargo.lock; windows.rs type-checks the Windows build from Linux
│   │
...
├── THIRD_PARTY_LICENSES.md               # generated by `cargo xtask licenses` (cargo-about + pnpm licenses); bundled into the app
...
- **`desktop/crates/xtask`** finds the workspace root by walking up to `Cargo.lock`, so it does not care how deep it sits. `gen-bindings` writes `desktop/frontend/src/bindings.ts`; `desktop/app/src/lib.rs` exports to `../frontend/src/bindings.ts` relative to the crate on every debug start and its `gen_bindings` test diffs against the same file. `licenses` writes `THIRD_PARTY_LICENSES.md` from the two lockfiles — Rust crates through cargo-about, JavaScript packages through `pnpm licenses`, and the fonts and marks copied in from development dependencies — and `--check`, which the release workflow runs, fails when it is out of date.
