...
**Neither Pages project deployed between 2026-09-07 and 2026-09-23** (found at the v0.1.0 release). Every build since the reorganisation that moved `website/` and `client-metadata/` under `web/` failed, for two reasons. The projects' root directories in Cloudflare still named the old folders — a dashboard setting, which the repository cannot change and the table above had been updated to describe as though it were done. And `pnpm install` inside `web/site` found the repository's own `pnpm-workspace.yaml` two levels up and installed the desktop workspace instead, so even the right folder would have built without Astro; `web/site/pnpm-workspace.yaml` makes the site its own workspace root. The live site in that fortnight was the build from before the move. The check to run after touching either project: the `Cloudflare Pages` check runs on the commit, which report failures here that nothing else surfaced.

**Domain.** The site is served from `gantry.oljo.dev`. *Moved off the apex 2026-09-18, on Olav's instruction, reversing the session-3 decision recorded as T19 in 01 §8 — the subdomain the original brief asked for turns out to be the one he wants, and it leaves `oljo.dev` itself free for whatever else lives there.* The apex redirects to it. The client-metadata document keeps its own host for the reason it always had: Pages serves one project per hostname, and the metadata URL is Gantry's OAuth identity, which must never move with a redesign (03 §7) — moving the marketing site is exactly the event that rule exists to survive, and `id.oljo.dev` did not move with it. The single-project alternative (serve it from `website/public/oauth/` at `https://gantry.oljo.dev/oauth/client-metadata.json`) is recorded and still available.
...
| `/download/` | One card per platform with a build — Windows and Linux since 2026-09-22, when macOS was left for later (09 M13) — with requirements, the stable asset names, coming-soon state; verify, build from source, then add a key |
...
**Releases.** `scripts/releases.ts` asks `api.github.com/repos/oljodev/gantry/releases/latest` once per page; with a release carrying the stable asset names (`Gantry-Windows-x64.exe`, `Gantry-Linux-x86_64.AppImage`, and `Gantry-macOS.dmg` once there is a macOS build) every `[data-dl]` button gets its `releases/latest/download/` URL, the hero's primary button points at the visitor's build — named only for a platform in `os.ts`'s `BUILT`, so a Mac visitor is sent to the download page rather than promised a file — and every version line fills in. Any failure leaves the coming-soon state, and every button still links somewhere useful (the download page, the releases page). The 404 in the console until then is expected.
