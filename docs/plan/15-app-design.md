...
| `ArtifactCard` | After the assistant text, one per artifact the turn created or changed: a 40 px `bg-raised` tile with the type glyph, the title in `ui` 500, "Markdown · v2" in `meta` `fg-3`, "Open" at the right; `r-3`, hairline, max 448 px; the whole card opens the panel |
