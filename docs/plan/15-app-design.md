...
| A7 | Activity | Inline in the assistant message, collapsed per turn behind a summary line; detail opens in the right pane | Reading order stays honest; matches 05 §1. The website's hero mock shows a side feed and is not the reference |
...
| A17 | Right side | One resizable pane (360 px to half the window) with tabs: artifacts, and tool-call or diff detail as a temporary tab | One mechanism; the chat never drops below its measure |
| A18 | Settings | Full page replacing the chat area: narrow section list on the left, stacked rows on the right (label, one-line explanation, control at the right edge); `/settings/$section` | Deep-linkable, room for the Providers and Connectors tables |
| A19 | Diffs | Row: first hunks inline, tinted lines, no gutter, mono 12 px. Pane: unified with line numbers by default, side-by-side toggle, Revert | Unified fits a 400 px pane; side-by-side is a choice, not a default |
...
| 1 | Raised: composer card, cards, inputs, user message block, connector tiles | `bg-raised` plus a `line-subtle` border; no shadow |
...
- **Right pane.** Opens from the right at 200 ms, pushes the chat column; at window widths under 1100 px it overlays the chat instead (level 3). Tabs at the top in `ui` 500: artifacts by title with their type icon, a temporary detail tab (diff, command, tool call, guard) in `fg-2` italic-free with a dot, closed with `×` or `Esc`. A toolbar row under the tabs belongs to the content (artifact toolbar per 13 §4; diff toolbar: unified/side-by-side, wrap, Revert).
- **Settings.** Replaces the chat column. Left list 200 px on `bg-base` with the section names in `ui`, current in `bg-selected`; right side on `bg-surface` at `measure` with a `page` title, then sections of rows. A row: label in `ui` 500, explanation in `meta` `fg-2` underneath, the control at the right edge (switch, select, button, or a key status). Tables (Providers, Connectors) are rows too, with the control column holding the actions.
...
| `SidebarItem`, `ChatRow`, `ProjectRow`, `DayGroup` | §7 sidebar; context menu on right-click and on a `⋯` that appears on hover |
| `UserMessage` | `bg-raised` block, `r-3`, 12 × 16 px padding, right-aligned, max 75 % of `measure`; attachments as thumbnails or file chips above the text |
| `AssistantMessage` | Plain `chat` text; markdown per 01 §5 with `CodeBlock` (level 0 on `bg-inset`, `r-3`, `code` size, language label and Copy in the top right on hover) |
| `TurnSummary` | One `ui` row at the top of a turn's activity: chevron, "7 tool calls · 3 files changed · 2 commands", elapsed; click or `→` expands |
...
| `InteractionCard` | The shared shell for permission, access request, connector suggestion, elicitation, auth required, skill and memory proposals (04 §7): level 1, `r-3`, a 2 px `accent` left bar, title in `ui` 500 with the connector mark, the request in `body`, a tier dot and label, then the actions row: primary action, secondary, Deny as ghost `bad`; a scope selector where grants apply. Pending cards also raise the sidebar count |
| `Composer`, `ModeChip`, `ModelPicker`, `RootChip`, `AttachmentTray`, `SlashMenu` | §7 composer; the mode chip is `control-sm`, `accent-subtle` fill with `accent-text` label when the mode is Auto, neutral otherwise; the guard state is a suffix ("Auto · guarded") |
| `PaneTabs`, `ArtifactToolbar`, `DiffView`, `CommandOutput`, `ToolCallDetail`, `JudgeDetail` | §7 right pane; `DiffView` is CodeMirror merge in unified mode with the diff tokens; `CommandOutput` renders ANSI on `bg-inset` in `code` |
