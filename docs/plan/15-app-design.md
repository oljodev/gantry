...
8. **A reading column never scrolls sideways.** Anything wider than the measure — a table, a code block, a diff, a shell command — scrolls inside its own box or is cut short with the whole of it a hover or a click away. The chat, the code session and every dialog section are read top to bottom; a horizontal scrollbar under a column of prose means something upstream refused to shrink, and the fix belongs there rather than on the scroller.
...
| A13 | Composer | Floating card at the bottom of the chat column: text on top, one toolbar row below (`+`, mode chip, model button, roots · thinking, Send). The `+` menu is files, folder, project, the connector checklist, web search — the provider's own where the model has it, otherwise the `web` connector, which the switch installs and attaches, and the row says which — and **thinking as a level** — off, low, medium, high, max — with the current one beside the label; the brain button still toggles, and turns back on at the last level in use | Mode and model are visible without a separate bar; the eye lands there on every screen. Thinking is a level because the field always was one, and a checkbox plus a level would be two controls for one decision: "how hard should it think" has an answer that is *not at all* (built 2026-09-13) |
...
| A15 | Palette | `Cmd/Ctrl+K` global palette: chats, messages, actions, settings sections, projects, connector install. Every row comes from the store or the catalog and every row goes somewhere: a project opens its page, a connector opens Customize at that connector, on the tab where it can be acted on | Widens the M2 search into the thing that makes the app keyboard-first. Until 2026-09-16 the projects and connectors here were the M0b fixtures — a project named "Gantry" nobody had, an "Install Slack" for a connector the catalog does not carry — and selecting a project did nothing at all, which is the worst of both: it looks like the feature works |
...
| A19 | Diffs | Row: first hunks inline, tinted lines, no gutter, mono 12 px, **Revert on hover**. Pane: unified with line numbers by default, side-by-side toggle, Revert. Both views carry **syntax colour** (shiki, the grammar from the file's extension) and a **stronger tint over the part of a replaced line that actually changed**. The same view serves the code surface's **Changes** tab, which lists every file a session touched with its net counts and shows the selected one underneath (16 §5) | Unified fits a 400 px pane; side-by-side is a choice, not a default. A whole-line tint says *a* line changed; on a long line with one renamed identifier, finding the change is still the reader's job, which is what the word-level tint answers |
| A20 | First launch | Three steps (add a key, pick a theme, optionally add a folder), then an empty chat with a short welcome, three suggested prompts and a hint to the `+` menu. The steps open when the flag in local storage is unset **and there are no chats**; the key step asks which provider and opens on the one the default model belongs to; the folder waits in `pendingRoots` until the first message makes a chat to attach it to. A turn that fails with `auth` offers **Add a key** | The empty state exists from day one instead of being retrofitted in M13. The second condition on the gate is what lets the flag live in local storage at all: an install with two years of chats must not be asked for its first API key the day that storage is cleared. Asking for an Anthropic key while the first message goes to OpenRouter was a setup step ending in a failed message, which is worse than none (built 2026-09-16) |
| A21 | Incognito | **Use incognito** beside the window controls (⌘/Ctrl ⇧ N) opens a chat **in the window you are in**: no memory read, no memory written, the memory tools dropped from the turn's tool set, absent from the sidebar, from search and from the artifact library, and deleted the moment you navigate away from it. Until its first message the screen is a ghost, the heading “You're incognito” and one line saying what it does not keep, with the composer between them; after it, the ordinary chat view. The ghost stays lit while you are in one. Skills still apply | Built first as a second OS window, on the reasoning that the answer to “is this being kept” should be which window you are looking at. Wrong twice in use: a second window is another thing to arrange on screen, and it leaves the app you were working in behind to do it. The screen says what the window was saying, and the ghost says it in the chrome. Skills are how the user works, not something the app learned about them (decided 2026-09-13, revised the same day) |
| A22 | Project page | The Projects list, then one project as five tabs — **Chats**, **Knowledge**, **Instructions**, **Defaults**, **Artifacts** — in the settings content column. **New chat** on the page starts one in the project, and says so above the composer before anything is sent. **Move to project…** on a chat row opens a picker that names what moving does and does not change, and a chat in a project carries its name as a composer chip beside the folder chips, linking to the page | The five tabs are the five things a project *is*, and a page that shows them all at once is a settings screen nobody reads. The sentence on the empty chat is there because a chat quietly inheriting instructions the user cannot see is the thing projects get wrong elsewhere (built 2026-09-13) |
| A23 | Chat menu | The `⋯` on a chat row carries what is set on that one conversation: **Instructions…** (10 §2 layer 6, a textarea with its 4,000-character count), **Permissions…**, **Move to project…**, beside rename, export and archive. The instructions dialog says which of the two things saving does — applied from the first message, or told to a chat that has already started | There is no chat settings panel and there does not need to be one: three dialogs off a menu beat a fourth place to look. The sentence about what saving does is there because the two look different from inside the conversation, and a note appearing in the transcript is not what a Save button promises (built 2026-09-13) |
| A24 | Terminal | A **Terminal tab** in the right pane of a session: the user's own login shell on a real pty, started in the session's folder, opened by `Ctrl/Cmd+\`` , by the palette's **Open terminal**, or by the terminal button in the pane's tab strip — never by the app, and never by a turn. Closing the tab ends the shell; switching away from it does not. Its sixteen colours are tokens like every other colour (§3) | The shell connector cannot be this and should not try: its input is closed so that a tool call can be captured and repeated (`shell.md` D3), which is exactly what a person typing needs it not to be. Asked for after a session where the model had run a dozen commands and there was no way to run the thirteenth by hand without leaving the window (built 2026-09-22) |
...
The sixteen ANSI colours a program running in the Terminal tab (A24) can ask for, plus the
cursor and the selection: `term-fg`, `term-bg`, `term-cursor`, `term-selection`, and
`term-{black,red,green,yellow,blue,magenta,cyan,white}` each with a `term-bright-` twin. They sit
in `tokens.css` with everything else, and the tab reads them from there at runtime so both
themes are one source — a terminal emulator carrying its own palette would be the one part of
the window that did not change with the theme.

Red, green, yellow and blue match `bad`, `good`, `warn` and `info`, because a red error is the
same red wherever it is printed. Magenta and cyan exist only here: nothing else in the app has a
sixth and seventh hue, and a palette missing two of sixteen would silently drop colours that
programs do use.
...
- **Title strip.** 38 px, part of every surface it crosses, `data-tauri-drag-region`. macOS: traffic lights at their native inset over `bg-base`, the sidebar's first row starts below them. Windows and Linux: three window controls drawn at the top right in `fg-2` **on `bg-inset`**, hover `bg-hover`, close hover `bad` with white glyph; double-click on the strip maximises. The ground under the three is what tells a frameless window where its own corner is — on the same ground as the strip they read as three glyphs floating in an empty bar. The strip takes an `actions` slot immediately to their left, on the strip's own ground: the main window puts **Use incognito** there (A21), and the windows that are already one document — onboarding, an artifact — pass nothing. The window's `backgroundColor` is set from `bg-base` before the webview paints (11 §3), so a new window never flashes white.
...
- **Chat column.** `bg-surface`, messages centred at `measure`. A turn is: user block, then the assistant text with its work folded inline (`TurnSteps`, A7): reasoning alone shows as the **thinking block** (one collapsed `ui` line in `fg-3` with a caret: "Thinking…" pulsing while it streams, "Thought for 4 s" after; expands to the reasoning at `ui` size behind a hairline); reasoning mixed with tool calls folds behind one `fg-2` line that names the work ("Created an artifact, read 3 files") or the current step ("Creating Dashboard…" with a 12 px spinner), and expands to the thinking blocks and activity rows in order behind a hairline. The spinner follows the turn, not the rows: once a turn has stopped the line names what actually ran ("Stopped before any work ran" when the answer was cut short before a call finished) and the cancelled rows say `cancelled`, so a stop always settles the line. A picture from an image model appears in the answer where the model produced it, bounded at 384 px and opening full size on a click, the same as one the user sent; sound from a model that talks or writes music appears as the browser's own player at `max-w-lg`, and a clip as a bounded `video` with controls and no autoplay. While a clip renders — half a minute to several — the provider notice under the composer says how long it has been going. After the text, an `ArtifactCard` per artifact the turn created or changed; then an optional interaction card, an inline error row in `bad-subtle` when the turn failed, and a `meta` footer on hover: when it ended, the model, and `⋯`, which opens a popover with the numbers — time, input, output, cached, sub-agent tokens, output speed, cost. **Behind `⋯` since 2026-09-22**: laid out in the row they were six figures under every answer, worth having and not worth reading every time. **Speed and cost added the same day:** speed is output tokens over the time from each round's first token to its last, the wait before the first token left out because that is queueing, not speed; cost is what the provider billed where it says (OpenRouter), and otherwise an estimate from the catalog's list prices marked `~` with a tooltip saying so (02 §8). Scroll pins to the bottom while streaming and releases the moment the user scrolls up — on the gesture, not on the distance travelled, so one wheel notch is enough — and pins again by itself as soon as they return to the bottom (within 24 px of it); sending a message pins again wherever they were, since their own message is about to appear there. A "↓ New" pill in `bg-overlay` sits at the bottom of the column for as long as the scroll is released, which is also how the user gets back.
...
- **Model dialog** (added 2026-09-08; the composer's model button opened a popover until then).
  ...
  header, the facets down a `--settings-list` rail on `bg-base` (**Makes**: text · image · speech ·
  music & audio · video, counted, and a kind with nothing in it is not shown at all; **Can**: reads images, reads files, calls tools, thinks first, caches prompts;
  ...
  **newest first** by default — it grows every week, and the model somebody is looking for is far
  ...
  and out, dollars per picture for a model that draws, or per million spoken tokens for one that
  talks, since a text token price says nothing about any of those. A video model shows no price
  at all: a clip costs by its seconds and its resolution, and the list carries neither, so the
...
- **Pages** (connector browse, connector detail, project) use the settings content column without the left list: `page` title, `body` intro, then content at `measure` or, for grids, the full width minus gutters.
...
| `ActivityRow` | 32 px min, icon 16 (connector mark or kind glyph in `fg-2`), title in `ui`, argument summary in `mono` `fg-3`, status at the right (spinner · `good` check · `bad` cross · elapsed in `meta`); variants edit (with `HunkPreview` below), command (last three output lines in `mono` on `bg-inset`, in a box that scrolls in itself and says how many lines are above them; `waiting` and `cancelled` are their own states, because a command nobody has answered for is not a failed one — it used to draw a `bad` cross with no exit code beside it and be counted in the fold's failures. No per-call Kill: the runner holds a cancellation token per call but nothing addresses one by id, so the button did nothing, and the composer's Stop is the control that works), connector call (progress bar 2 px `accent` when reported), read/search, guard mark (a `good` shield in the status cluster of any row the guard allowed — a command above all, which is what it usually decides about — carrying its reason on hover; a block is a `bad-subtle` row of its own, "Blocked by guard: reason" with Allow anyway, because it stopped the work), notice (`info` icon, `meta`), artifact ("Created artifact · Title" with the type icon; opens the pane), context ("2 skills, 5 memories"). One line, always: whichever of the title and the summary is the long one is the one that truncates, and whatever the row cut short is on hover and in the pane. A call that failed puts the message it returned where its arguments would have been, in `bad` at `ui` size — the fold above it counts the failures and only the row can say what they were |
...
| `ConnectorMark` | The mark beside a connector anywhere it is named: a Phosphor glyph for Gantry's own four, otherwise the service's own mark from `src/lib/marks.generated.ts` (`pnpm marks`; Simple Icons where the vendor is carried there, the connector folder's `icon.svg` where it is not), drawn at `size` in the vendor's own colour, which is the one exception to `tokens.css` holding every colour in the app: a brand mark is data about somebody else, generated rather than written, and a wall of sixty monochrome logos is harder to search than a wall of coloured ones. `pnpm marks` picks two versions of each colour, one per theme, pushing it towards white or black until it clears 3:1 against that theme's surface — GitHub's near-black would otherwise vanish into the dark ground and Hugging Face's yellow into the light one — and `.brand-mark` in `globals.css` chooses between them with tokens.css's own three-way theme structure. Gantry's own glyphs stay in `currentColor`. A two-letter monogram is what is left for a server the user added by hand, which is the only thing the catalogue has never heard of. An instance namespaced `github-2` (03 §6) is still GitHub |
| `InteractionCard` | The shared shell for permission, access request, connector suggestion, elicitation, auth required, skill and memory proposals (04 §7): level 1, `r-3`, a 2 px `accent` left bar, title in `ui` 500 with the connector mark, the request in `body`, a tier dot and label, then the actions row: primary action, secondary, Deny as ghost `bad`; a scope selector where grants apply. Pending cards also raise the sidebar count. A card may also carry **choices** (04 §7): one labelled row per changeable argument, using the `OptionPicker` above, so a model menu behaves the same on the card as in the connector's own settings — the value on the left, its release month and rate on the right, and a filter box once the list is long. Where the resolved value is not what the model asked for, a `warn` line says so. The argument the menu is about is dropped from the card's own argument list: one question, one answer, in one place. As built it carries five: `PermissionCard`, `AccessRequestCard` ("GitHub in this chat?", the model's reason, **Attach for this chat** · **Attach and allow …** · **Not now**), `ConnectorSuggestionCard` ("Install Cloudflare Workers?", the reason, badges for what it needs, **Install** · **Not now**), and the two proposals of M12. `SkillProposalCard` is a **form**: name, description and body are all editable before **Save skill**, because a skill written mid-conversation is a first draft of something kept for months; a collision says which version it would replace, offers the free name beside it, and refuses outright to replace a bundled skill. `MemoryProposalCard` is one editable line (**Remember** · **Not this**), which is what makes confirming it cheap rather than annoying; with auto-save on it reads **Remembered** and offers **Undo** instead |
...
| `Composer`, `ModeChip`, `ModelPicker` (the button that opens the model dialog — the composer's, and the control every other place that chooses a model uses: a settings form picking between four hundred models is asking the same question), `RootChip`, `SlashMenu` | §7 composer; the mode chip is `control-sm`, `accent-subtle` fill with `accent-text` label when the mode is Auto, neutral otherwise; the guard state is a suffix ("Auto · guarded") |
...
| Follow and room (2026-09-22) | The feed follows its bottom while the reader is there and lets go the moment they scroll **up** — by direction, not by position, because a notch of the wheel near the bottom is still "at the bottom" and re-latching on that made nudging the feed impossible. Coming back down to the bottom follows again. The newest turn is given the rest of the window under it (the scroller's height less its own, less 72 px), so a message just sent sits near the top with room for the answer to grow into; the room shrinks as it fills and is gone once the turn is taller than the window |
...
Contrast per §3. Full keyboard operation: every action reachable from the palette or a documented shortcut (`Cmd/Ctrl+N` new chat, `Cmd/Ctrl+Shift+N` a private one, `Cmd/Ctrl+K` palette, `Cmd/Ctrl+Shift+K` the other surface, `Cmd/Ctrl+B` sidebar, `Cmd/Ctrl+,` settings, `Cmd/Ctrl+Shift+A` toggle the right pane, `Cmd/Ctrl+\`` the terminal (A24), `Cmd/Ctrl` with `+`/`-`/`0` zoom the artifact box the panel is showing and nothing else (13 §4), `Shift+Tab` mode — in the composer, which costs it its backwards focus step, the trade Claude Code makes too — `Esc` closes the topmost thing). Every shortcut the window *draws* is written the way the running platform writes it (`lib/shortcuts.ts`): `⌘⇧K` on macOS, `Ctrl+Shift+K` everywhere else. Until 2026-09-16 the sidebar, the palette, the welcome hint and two tooltips drew `⌘` on every machine while the app listened for Ctrl on all but one — a label for a key combination that does nothing, and with macOS dropped (09 M13) it was wrong on every machine Gantry runs on. Minimum target 24 px. Focus ring on everything. Live regions: streaming text is `aria-live="off"` with a polite announcement when a turn completes or needs a decision. Icon buttons labelled. Dialogs trap focus and restore it. Colour is never the only carrier of meaning: tiers and statuses have labels, diffs have `+`/`−` prefixes.
...
- **13 §4.** The artifact panel is the right pane; each open artifact is a closable tab, and its one-row toolbar (Rendered | Source as glyphs, the version stepper when there is more than one version, the zoom readout once it is not 100 %, Restore and Fix this when they apply, Copy, and a menu with zoom, Download, Open in window and Edit source) sits at the top of the tab's content with the Problems strip at the bottom.
