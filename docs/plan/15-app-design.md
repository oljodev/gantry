...
| A21 | Incognito | **Use incognito** beside the window controls (⌘/Ctrl ⇧ N) opens a chat in its own window: no memory read, no memory written, the memory tools dropped from the turn's tool set, absent from the sidebar, from search and from the artifact library, and deleted when the window closes. Its window carries a ghost, the heading “You're incognito” and one line saying what it does not keep. Skills still apply | A private conversation that shares a sidebar with the history is one keystroke from being in it. Making it a window rather than a mode means the answer to “is this being kept” is which window you are looking at. Skills are how the user works, not something the app learned about them (decided 2026-09-13) |
