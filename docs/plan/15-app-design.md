...
- **Chat column.** `bg-surface`, messages centred at `measure`. A turn is: user block, an optional **thinking block** (one collapsed `ui` line in `fg-3` with a caret: "Thinking…" pulsing while it streams, "Thought for 4 s" after; expands to the reasoning at `ui` size behind a hairline; added in M1), assistant text with inline activity, an optional interaction card, an inline error row in `bad-subtle` when the turn failed, and a `meta` footer on hover (model, duration, tokens). Scroll pins to the bottom while streaming and releases the moment the user scrolls up; a "↓ New" pill in `bg-overlay` appears at the bottom when new content arrives while released.
