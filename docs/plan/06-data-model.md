...
- **agent_types** — `id` (the slug a model names in a call), `name, description, instructions, model` (`inherit` | `rules` | `provider/model`), `connectors_json, mode NULL, guard NULL, write_files, memory, skills, open_json` (which fields the parent may set), `builtin, enabled, created_at, updated_at` (18 §3, migration 0016). The two shipped types are rows written by the migration, not constants in code, so that editing one is an ordinary update; `builtin` is what makes **Reset** possible and what stops a delete
