...
- `SEARCH_PROVIDER` — `brave`, `tavily` or `exa`.
...
test` never opens a socket — the refusal tests in `tests/tools.rs` in particular prove their
point only because no request is made. The one live test is `#[ignore]`d and reads its key from
the environment:
